...
public class OrdersProgressFragment extends Fragment implements View.OnClickListener {
    ...
    public void initView() {
        ...
        getOrdersProgressOnNext = new SubscriberOnNextListener<List<OrdersProgress>>() {
            @Override
            public void onNext(List<OrdersProgress> progressPage) {
    ...
    private void addProgressItem(OrdersProgress progress) {
        ...
        } else if (rightBtn.getText().toString().equals("上门")) {
            rightBtn.setEnabled(true);
    ...
    public void onClick(View view) {
       ...
       Intent intent = new Intent(activity, ChatActivity.class);
                 System.out.println(orders.getUser().getAccount());
                 intent.putExtra(IMDebugApplication.TARGET_ID, orders.getUser().getAccount());
            ...
            case R.id.btn_right:
                if (rightBtn.getText().toString().equals("接单")) {
                    changeState("已接单", "请卖方尽快上门", 2);
                    rightBtn.setEnabled(false);
                } else if (rightBtn.getText().toString().equals("上门")) {
                    rightBtn.setEnabled(false);
                    changeState("已上门", "请保证联系方式有效", 3);
                }
...
//                    activity.startActivity(itnt);
                    ...
                    rightBtn.setEnabled(false);
                    changeState("同意退款", "金币（在线支付）已退回买方", 7);
                }
